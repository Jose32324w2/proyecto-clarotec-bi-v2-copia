#!/usr/bin/env python
"""Django's command-line utility for administrative tasks."""
import os  # Importa os
import sys  # Importa sys


def main():
    """Run administrative tasks."""
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'clarotec_api.settings')  # Configura la configuración de Django
    try:
        from django.core.management import execute_from_command_line  # Importa execute_from_command_line
    except ImportError as exc:
        raise ImportError(
            "Couldn't import Django. Are you sure it's installed and "
            "available on your PYTHONPATH environment variable? Did you "
            "forget to activate a virtual environment?"
        ) from exc
    execute_from_command_line(sys.argv)  # Ejecuta el comando


if __name__ == '__main__':  # Si es el archivo se ejecuta directamente
    main()
